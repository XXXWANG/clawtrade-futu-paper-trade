...
def ensure_venv():
    ...
    try:
        ...
        json_out({"ok": False, "error": f"启动虚拟环境进程失败: {exc}"}, 1)
    sys.exit(completed.returncode)


def configure_runtime_logging():
    if os.environ.get("FUTU_VERBOSE_LOGS") == "1":
        return
...
def parse_trd_env(trd_env):
    ...
    if env in {"PAPER", "SIMULATE", "SIM"}:
        return TrdEnv.SIMULATE
    json_out(
        {
            "ok": False,
            "error": "无效的 FUTU_TRD_ENV",
            "next_steps": [
                "请设置 FUTU_TRD_ENV 为 PAPER 或 SIMULATE",
                "示例: export FUTU_TRD_ENV=SIMULATE",
            ],
        },
        1,
    )
...
def parse_trd_market(trd_market):
    ...
    json_out(
        {
            "ok": False,
            "error": "无效的 FUTU_TRD_MARKET",
            "next_steps": [
                "请设置 FUTU_TRD_MARKET 为 HK/US/CN/HKCC/USCC",
                "示例: export FUTU_TRD_MARKET=HK",
            ],
        },
        1,
    )
...
def get_port():
    raw_port = get_env("FUTU_PORT", "11111")
    try:
        port = int(raw_port)
    except (TypeError, ValueError):
        json_out({"ok": False, "error": "FUTU_PORT 必须是数字端口"}, 1)
    if port < 1 or port > 65535:
        json_out({"ok": False, "error": "FUTU_PORT 超出合法范围(1-65535)"}, 1)
    return port
...
def parse_positive_int(value, field_name):
    try:
        ...
        json_out({"ok": False, "error": f"{field_name} 必须是整数"}, 1)
    if parsed <= 0:
        json_out({"ok": False, "error": f"{field_name} 必须大于 0"}, 1)
    return parsed
...
def parse_positive_float(value, field_name):
    try:
        ...
        json_out({"ok": False, "error": f"{field_name} 必须是数字"}, 1)
    if parsed <= 0:
        json_out({"ok": False, "error": f"{field_name} 必须大于 0"}, 1)
    return parsed
...
def parse_non_negative_float(value, field_name):
    try:
        ...
        json_out({"ok": False, "error": f"{field_name} 必须是数字"}, 1)
    if parsed < 0:
        json_out({"ok": False, "error": f"{field_name} 不能小于 0"}, 1)
    return parsed
...
def open_contexts(host, port, trd_market):
    ...
    try:
        ...
        json_out(
            {
                "ok": False,
                "error": f"连接 FutuOpenD 失败: {exc}",
                "next_steps": [
                    "请确认 FutuOpenD 已启动并保持运行",
                    "确认 FUTU_HOST 与 FUTU_PORT 配置正确",
                    "可先执行 check 指令进行连通性检查",
                ],
            },
            1,
        )
...
def close_contexts(quote_ctx, trade_ctx):
    ...
    if quote_ctx is not None:
        try:
            ...
            close_errors.append(f"quote_ctx.close() 失败: {exc}")
    if trade_ctx is not None:
        try:
            trade_ctx.close()
        except Exception as exc:
            close_errors.append(f"trade_ctx.close() 失败: {exc}")
    if close_errors and os.environ.get("FUTU_VERBOSE_LOGS") == "1":
        logging.getLogger(__name__).warning("; ".join(close_errors))
...
def call_ctx_method(handler, param_groups, fallback_args):
    ...
    if signature:
        ...
        if params and all(name in params for name in required_params):
...
def unlock_trade(trade_ctx):
    ...
    if not password:
        json_out(
            {
                "ok": False,
                "error": "缺少 FUTU_TRADE_PWD",
                "next_steps": [
                    "请设置交易解锁密码 FUTU_TRADE_PWD",
                    "不要使用 FUTU_PASSWORD 作为交易密码替代",
                ],
            },
            1,
        )
...
def cmd_order(args, side):
    ...
    trd_market = parse_trd_market(get_env("FUTU_TRD_MARKET", "HK"))
    symbol = args.symbol
    qty = parse_positive_int(args.qty, "qty")
    order_type = args.order_type.lower().strip()
    if order_type not in {"limit", "market"}:
        json_out({"ok": False, "error": "order-type 仅支持 limit 或 market"}, 1)
    if order_type == "market":
        price = parse_non_negative_float(args.price, "price") if args.price is not None else 0.0
    else:
        if args.price is None:
            json_out({"ok": False, "error": "limit 订单必须提供 --price"}, 1)
        price = parse_positive_float(args.price, "price")
...
def cmd_fills(args):
    ...
    trd_env = parse_trd_env(get_env("FUTU_TRD_ENV", "SIMULATE"))
    trd_market = parse_trd_market(get_env("FUTU_TRD_MARKET", "HK"))
    days = parse_positive_int(args.days, "days")
    _, _, TrdEnv, _, _, _, _, _ = load_futu()
    quote_ctx, trade_ctx = open_contexts(host, port, trd_market)
    try:
        if days <= 1:
